  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (dept?.nombre) {
        ...
        const { data: emps } = await supabase.from('usuarios').select('*').eq('departamento', dept.nombre).neq('rol', 'admin').order('nombre')
        setEmpleados(emps || [])
      } else {
        setEmpleados([sup])
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">Vista completa de ausencias del departamento</p>
          </div>
        ...
        {empleadosFiltrados.length === 0 ? (
          ...
          <div className="bg-white rounded-xl shadow overflow-x-auto">
            <div className="px-4 py-3 bg-gray-50 border-b">
              <h3 className="font-semibold text-gray-700">{departamento}</h3>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  {dias.map((d, i) => (
                    <th key={i} className="px-4 py-3 text-gray-600 font-semibold text-center">{formatFecha(d)}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
