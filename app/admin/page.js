...
export default function Admin() {
  ...
  const handleSubmitAusencia = async (e) => {
    ...
    if (conflictos?.length > 0) {
      setMensajeAus('Ya tenes ausencias registradas en: ' + conflictos.map(c => new Date(c.fecha).toLocaleDateString('es-AR')).join(', '))
      setLoadingAus(false)
      return
    }
  ...
  const handleGuardarEdicionAus = async (ids) => {
    ...
    if (reales.length > 0) {
      setMensajeAus('Ya tenes ausencias en: ' + reales.map(c => new Date(c.fecha).toLocaleDateString('es-AR')).join(', '))
      return
    }
    const bsas = getBsasTime()
    await supabase.from('ausencias').delete().in('id', ids)
