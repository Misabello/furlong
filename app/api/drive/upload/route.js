...
async function getAdminInfo() {
  ...
  if (new Date(data.google_token_expiry) > new Date()) {
    return { token: data.google_access_token, email: data.email }
  }
...
export async function POST(request) {
  try {
    ...
    if (!adminInfo) return Response.json({ ok: false, reason: 'no_admin_token' }, { status: 401 })
    ...
    if (!uploadRes.ok) {
      ...
      return Response.json({
        ok: false,
        reason: err?.error?.message || 'upload_error',
    ...
    return Response.json({ ok: false, reason: e.message || 'server_error' }, { status: 500 })
  }
}
