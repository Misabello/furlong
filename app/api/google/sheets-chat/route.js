...
async function getAccessToken(usuario) {
  ...
  if (new Date(usuario.google_token_expiry) > new Date()) return usuario.google_access_token
...
export async function POST(request) {
  const { userId, titulo, headers, rows } = await request.json()
  ...
  const rowData = allRows.map(fila => ({
  ...
  const res = await fetch('https://sheets.googleapis.com/v4/spreadsheets', {
    ...
    body: JSON.stringify({
      properties: { title: titulo || 'Datos del asistente' },
      sheets: [{ properties: { title: 'Datos' }, data: [{ startRow: 0, startColumn: 0, rowData }] }]
    }),
  })
