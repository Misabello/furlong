...
export default function Empleado() {
  ...
  const tieneAusenciaCal = (empleadoId, fecha) => {
    const fechaStr = toLocalISO(fecha)
    return ausenciasCal.find(a => a.empleado_id === empleadoId && a.fecha === fechaStr)
  }

  const getAdjuntoCal = (empleadoId, fecha) => {
    const fechaStr = toLocalISO(fecha)
    return adjuntosCal.find(a => a.empleado_id === empleadoId && a.fecha_desde <= fechaStr && (a.fecha_hasta || a.fecha_desde) >= fechaStr)
  }

  const formatFechaCal = (d) => d.toLocaleDateString('es-AR', { weekday: 'short', day: 'numeric', month: 'short' })
  const getCatCal = (nombre) => categorias.find(c => c.nombre === nombre) || { emoji: '📝', color: 'bg-gray-100 text-gray-600' }
  ...
  useEffect(() => {
    ...
    let query = supabase.from('ausencias').select('*').in('empleado_id', ids).gte('fecha', fechaInicioCal).lte('fecha', fechaFinCal)
    if (filtroMotivoCal !== 'todos') query = query.eq('motivo', filtroMotivoCal)
    Promise.all([
      query,
      fetch(`/api/adjuntos?empleadoIds=${ids.join(',')}`).then(r => r.json())
    ]).then(([{ data }, adj]) => {
      if (!activo) return
      setAusenciasCal(data || [])
      setAdjuntosCal(adj || [])
    }).catch(() => {})
    return () => { activo = false }
            ...
            <div className="bg-white rounded-xl shadow px-4 py-3 mb-4">
              <div className="flex flex-wrap gap-3 items-end">
                <div>
                  ...
                  <input type="text" value={busquedaUsuarioCal} onChange={e => setBusquedaUsuarioCal(e.target.value)} placeholder="Buscar por nombre..." className="border border-gray-300 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 w-44" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Tipo de ausencia</label>
                  <select value={filtroMotivoCal} onChange={e => setFiltroMotivoCal(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="todos">Todos</option>
                    {categorias.map(c => <option key={c.id} value={c.nombre}>{c.emoji} {c.nombre}</option>)}
                  </select>
                </div>
                <button onClick={() => { setModoFiltroCal(!modoFiltroCal); setFiltroDesde(''); setFiltroHasta('') }} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${modoFiltroCal ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
                  Filtrar por fecha
                </button>
                {modoFiltroCal && (
                  <>
            ...
            <div className="bg-white rounded-xl shadow overflow-x-auto">
              {(() => {
                const empConEventos = empleadosTodos
                  .filter(emp => !busquedaUsuarioCal.trim() || emp.nombre?.toLowerCase().includes(busquedaUsuarioCal.toLowerCase().trim()))
                  .filter(emp => diasMostrarCal.some(d => tieneAusenciaCal(emp.id, d)))
                ...
                return (
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {diasMostrarCal.map((d, i) => (
                        <tr key={i} className="border-b hover:bg-gray-50">
                          <td className="px-3 py-2 font-medium text-gray-600 sticky left-0 bg-white whitespace-nowrap z-10">{formatFechaCal(d)}</td>
                          {empConEventos.map(emp => {
                            const ausencia = tieneAusenciaCal(emp.id, d)
                            const cat = ausencia ? getCatCal(ausencia.motivo) : null
                            const adjunto = ausencia ? getAdjuntoCal(emp.id, d) : null
                            return (
